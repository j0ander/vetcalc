...
<header class="w-full top-0 sticky bg-surface/90 backdrop-blur-sm shadow-sm flex justify-between items-center px-3 sm:px-container-padding h-touch-target-min z-50 transition-colors duration-200">
  <div class="flex items-center gap-1.5 sm:gap-2 min-w-0">
    ...
    <h1 class="font-headline-md text-[17px] sm:text-headline-md font-bold text-primary truncate">Historial</h1>
...
<main class="relative z-10 pb-24 pt-3 sm:pt-4 px-3 sm:px-container-padding max-w-2xl mx-auto">

  <!-- ===== FILTROS ===== -->
  <section class="mb-3 sm:mb-4 flex gap-1.5 sm:gap-2 overflow-x-auto pb-2 scrollbar-hide animate-fade-in-up" id="filter-buttons">
    <button class="filter-btn px-3 sm:px-4 py-1.5 sm:py-2 rounded-full bg-secondary text-white font-label-md text-[11px] sm:text-label-md whitespace-nowrap shadow-sm hover:shadow-md transition-all" data-filter="all">Todos</button>
    <button class="filter-btn px-3 sm:px-4 py-1.5 sm:py-2 rounded-full bg-white/80 backdrop-blur-sm text-on-surface-variant font-label-md text-[11px] sm:text-label-md whitespace-nowrap hover:bg-primary/10 hover:text-primary transition-all border border-outline-variant" data-filter="dosage">Dosis</button>
    <button class="filter-btn px-3 sm:px-4 py-1.5 sm:py-2 rounded-full bg-white/80 backdrop-blur-sm text-on-surface-variant font-label-md text-[11px] sm:text-label-md whitespace-nowrap hover:bg-primary/10 hover:text-primary transition-all border border-outline-variant" data-filter="fluidotherapy">Fluidos</button>
    <button class="filter-btn px-3 sm:px-4 py-1.5 sm:py-2 rounded-full bg-white/80 backdrop-blur-sm text-on-surface-variant font-label-md text-[11px] sm:text-label-md whitespace-nowrap hover:bg-primary/10 hover:text-primary transition-all border border-outline-variant" data-filter="anesthesia">Anestesia</button>
  </section>

  <!-- ===== CONTENEDOR DE HISTORIAL ===== -->
  <div class="bg-white rounded-xl sm:rounded-2xl border border-outline-variant shadow-md p-3 sm:p-4 animate-fade-in-up">
    <div class="space-y-2 sm:space-y-3" id="history-list">
  ...
  <div class="mt-5 sm:mt-6 text-center">
    <button class="text-primary font-label-md text-[12px] sm:text-label-md border-b-2 border-primary-fixed-dim pb-0.5 sm:pb-1 hover:border-primary transition-all hover:scale-105" id="load-more-btn">
...
<div id="history-detail-modal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm hidden animate-fade-in-up">
  <div class="bg-white/90 backdrop-blur-sm rounded-xl sm:rounded-2xl p-4 sm:p-6 max-w-lg w-full mx-3 sm:mx-4 shadow-2xl border border-outline-variant max-h-[90vh] overflow-y-auto">
    <div class="flex justify-between items-center mb-3 sm:mb-4">
      <h3 class="font-headline-md text-[17px] sm:text-headline-md text-on-surface">Detalle del Cálculo</h3>
      <button id="close-history-detail-btn" class="p-1.5 sm:p-2 rounded-full hover:bg-surface-container-high transition-colors">
        <span class="material-symbols-outlined text-xl sm:text-2xl">close</span>
      </button>
    </div>

    <div id="detail-content" class="space-y-2 sm:space-y-3 text-[14px] sm:text-base">
      <!-- Todo se inyecta dinámicamente desde el controlador -->
    </div>

    <button id="delete-history-btn" class="mt-3 sm:mt-4 w-full bg-error text-white h-12 sm:h-touch-target-min rounded-xl font-label-md text-[13px] sm:text-label-md shadow-sm hover:shadow-md transition-all flex items-center justify-center gap-1.5 sm:gap-2">
      <span class="material-symbols-outlined text-xl sm:text-2xl">delete</span>
      Eliminar registro
    </button>
  </div>
</div>
...
<button class="fixed bottom-24 right-4 sm:right-6 h-12 w-12 sm:h-14 sm:w-14 rounded-full bg-primary text-white shadow-lg hover:shadow-xl active:scale-90 transition-all duration-300 z-40 flex items-center justify-center hover:-translate-y-1" id="search-fab">
  <span class="material-symbols-outlined text-[22px] sm:text-[28px]">search</span>
</button>
...
<nav class="fixed bottom-0 left-0 right-0 flex justify-around items-center px-2 pb-safe bg-surface/90 backdrop-blur-sm dark:bg-surface h-[60px] sm:h-[68px] z-50 shadow-[0px_-4px_20px_rgba(0,0,0,0.06)]">
  ...
  <a class="relative flex flex-col items-center justify-center text-primary font-bold border-t-2 border-primary pt-2 -translate-y-1 shadow-sm transition-all duration-300 ease-out hover:-translate-y-1.5 hover:shadow-md active:scale-95 group" href="#" data-route="history">
    <span class="material-symbols-outlined text-[22px] sm:text-[24px] transition-transform duration-300 group-hover:scale-110" style="font-variation-settings: 'FILL' 1;">history</span>
